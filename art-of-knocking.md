...
- Climbed: Director of Training → Director of Sales → Director of Rep Experience
  (events, gear, design, blog) → Senior Director of Business Development (built "the
  Collective") → Executive Director over the entire program before exit.

## The ethos (craft)

The vision: the craft passes generation to generation with **dignity and
integrity**. The personal arc that defines it:

- Memorize the pitch → **internalize** the pitch
...
## The sales process (craft made explicit)

Document the sales process with diagrams and the psychology behind it: prospecting
→ qualifying → presenting → overcoming objections → closing. Capture the objection
handling and the "why" behind each move.
...
- **Qual card + word-for-word pitch teardown** → gets its **own file** (a pitch
  breakdown is a different artifact than a brand story). Referenced from here.
- **GCCO** (governance, cadence, compensation, objectives, key results) → lives in
  [Statement](/statement) under executive management, **not** here.
- Territory craft feeds Hexpin.
